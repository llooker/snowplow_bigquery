...
# Funnel Fields #
  ...
  - measure: funnel.event_1_count_sessions
    type: number
    sql: COUNT(DISTINCT CASE WHEN {% condition event_1 %} ${event_type} {% endcondition %} THEN CONCAT(${user.domain_userid},  CAST(${domain_session_index} as string)) END)

  - measure: funnel.event_2_count_sessions
    type: number
    sql: COUNT(DISTINCT CASE WHEN {% condition event_2 %} ${event_type} {% endcondition %} THEN CONCAT(${user.domain_userid},  CAST(${domain_session_index} as string)) END)

  - measure: funnel.event_3_count_sessions
    type: number
    sql: COUNT(DISTINCT CASE WHEN {% condition event_3 %} ${event_type} {% endcondition %} THEN CONCAT(${user.domain_userid},  CAST(${domain_session_index} as string)) END)
